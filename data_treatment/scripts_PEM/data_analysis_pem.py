...
def summarize_sweep(cutoff_voltage):
    ...
    usable = sweep[sweep["pem_voltage_V"] >= cutoff_voltage]
    collapse = sweep[sweep["pem_voltage_V"] < cutoff_voltage]
...
def get_polarization_limit(polarization_summary):
    ...
    usable = polarization_summary[
        (polarization_summary["is_increasing_current"])
        & (polarization_summary["is_above_cutoff"])
        & (polarization_summary["power_W"] > 0)
    ].copy()

    if len(usable) == 0:
        return np.nan, np.nan

    max_power_idx = usable["power_W"].idxmax()
    row = usable.loc[max_power_idx]
...
def make_control_parameters(
    ...
    if pd.isna(max_discharge_current_a):
        max_discharge_current_a = float(sweep_summary.iloc[0]["max_sustainable_current_A"])

    if pd.isna(max_discharge_power_w):
        max_discharge_power_w = float(sweep_summary.iloc[0]["max_sustainable_power_W"])
    ...
    return pd.DataFrame([
        {
            ...
            "polarization_charge_hydrogen_mL_per_input_J": (
...
def summarize_polarization_curve(cutoff_voltage=None):
...
def estimate_cutoff_from_polarization(polarization):
    if len(polarization) == 0:
        return FALLBACK_CUTOFF_VOLTAGE

    usable_curve = polarization[
        (polarization["is_increasing_current"])
        & (polarization["current_A"] > 0)
        & (polarization["voltage_V"] > 0)
        & (polarization["power_W"] > 0)
    ].copy()

    if len(usable_curve) == 0:
        return FALLBACK_CUTOFF_VOLTAGE

    max_power_idx = usable_curve["power_W"].idxmax()
    return float(usable_curve.loc[max_power_idx, "voltage_V"])
...
def plot_polarization_power_curve(cutoff_voltage):
    polarization = summarize_polarization_curve(cutoff_voltage)
    if len(polarization) == 0:
        return
    ...
    mpp_idx = increasing["power_W"].idxmax()
    mpp = increasing.loc[mpp_idx]
    ...
    voltage_mpp = voltage_ax.scatter(
        mpp["current_A"],
        mpp["voltage_V"],
        s=105,
        color=GREEN,
    ...
    power_mpp = power_ax.scatter(
        ...
        zorder=5,
        label=(
            f"MPP: {mpp['current_mA']:.1f} mA, "
            f"{mpp['voltage_V']:.3f} V, "
            f"{mpp['power_mW']:.1f} mW"
        ),
    )
    voltage_ax.axvline(
        mpp["current_A"],
    ...
    voltage_ax.set_title("PEM Polarization Cutoff from Maximum Power Point")

    cutoff_handle = voltage_ax.get_lines()[-1]
    handles = [voltage_line, power_line, power_mpp, cutoff_handle]
    labels = [
        "Voltage",
        "Power",
        (
    ...
    fig.savefig(PLOT_DIR / "pem_polarization_cutoff_mpp.png", bbox_inches="tight", facecolor="white")
    plt.close(fig)
...
def save_outputs(
    ...
    if len(polarization_charge_summary) > 0:
        polarization_charge_summary.to_csv(
...
def make_plots(charge_summary, cutoff_voltage, full_cycle_charge, full_cycle_discharge, full_cycle_summary):
    ...
    plot_polarization_power_curve(cutoff_voltage)
